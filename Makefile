...
test-integration:
	cd server && test -n "$(SUPPQ_TEST_DATABASE_URL)" && GOCACHE=$(GOCACHE) SUPPQ_TEST_DATABASE_URL="$(SUPPQ_TEST_DATABASE_URL)" go test ./internal/identity ./internal/catalog -count=1 -v
