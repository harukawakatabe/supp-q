  ...
  <view class="page"><view class="bar"><button class="plain" @click="back">{{mode==='choose'?'取消':'‹ 返回'}}</button><text class="bar-title">添加产品</text><view/></view>
    <main class="content">
      ...
      <template v-else>
        ...
        <section class="card"><text class="section-title">每周计划</text><label><text>开始日期</text><input v-model="form.startDate"/></label><view class="weekday-row"><button v-for="day in weekdays" :key="day.v" class="weekday" :class="{'weekday--active':form.selectedWeekdays.includes(day.v)}" @click="toggleWeekday(day.v)">{{day.l}}</button></view><label><text>计划时点</text><input v-model="form.reminderTime"/></label><text class="hint">用于生成今日任务，不代表通知授权。</text></section>
