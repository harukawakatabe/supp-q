...
| D-034 | E4 treats legacy `recognition_jobs` as a compatibility queue only. Target recognition jobs bind exactly one CaptureSlotVersion, attempts are separate identities, evidence/candidates are append-only per attempt, and only a terminal current-attempt/current-version candidate may update an editable ConfirmationDraft. | The legacy `(recognition_set_id, role)` uniqueness cannot represent image replacement. Separate target Job/Attempt identities prevent cross-aggregate test fixtures and ensure lease reclaim or replaced-image late results remain evidence rather than confirmed facts. Multiple active legacy sets are quarantined, not silently cancelled. |
| D-035 | E5 keeps legacy Intake/Batch reads compatible while making InventoryEvent source identity and compensation append-only, storing as-allocated batch/profile snapshots, and rejecting the same intake idempotency key when its normalized request hash differs. Historical rows receive migration provenance but no fabricated ClientAction/request hash; legacy zero prices are not interpreted as free. | Exact undo and replay require the original batch/event relationship, while an idempotency key without payload equality is unsafe. The current endpoint has only a free-form idempotency key, so fabricating UUID ClientActions would create false audit history. R2 remains the owner of cost semantics. |
