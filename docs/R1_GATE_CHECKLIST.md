...
| RG1 Repeatable build | `PARTIAL` | Main baseline hosted CI and E2–E6 local checks pass; the owner selected no PR, the development branch has no hosted run, and enforceable branch rules/scans/promotion evidence remain open |
| RG2 Migration safe | `PARTIAL` | Platform spine through E6 Risk/Reminder pass local fresh/synthetic-current/restart/rollback guards; scale/lock, C1 and production-like evidence remain open |
| RG3 Domain/security correct | `PARTIAL` | E5 locally verifies replay, exact compensation and intake retry concurrency on synthetic data; complete S7 and R1-applicable Q1–Q9 coverage do not exist |
...
The workflow runs on PRs and pushes to `main`, so the current E2–E6 branch has
...
- [x] Empty database `up` succeeds through platform spine, E2 Product/Profile, E3 ProductPlan/ScheduleVersion, E4 Capture/Evidence, E5 Intake/Inventory, and E6 Risk/Reminder.
- [x] A representative synthetic current Launch-Beta snapshot upgrades through E6 without reset.
...
- [x] E5 Intake/Inventory catch-up is phased, cursor-based, idempotent, restartable, catches an N-1 write, and exposes attempts/source/hash/progress timestamps.
- [x] E6 conservatively catches N-1 Workspaces, preserves unconfirmed consent, and provides complete-revision and monotonic replay-cursor contracts; the S8 materializer remains open.
...
- [x] E5 intake/inventory replay, event snapshot, allocation, tenant, state-chain, and compensation reconciliation passes locally.
- [x] E6 risk/reminder consent, revision, tenant, dedupe, channel, target, and cursor reconciliation executes locally.
...
E5 implementation commit `14b0178`,
E6 implementation commit `0a54f1e`,
...
`../reports/r1/2026-09-26-s1-intake-inventory/README.md`, and
`../reports/r1/2026-09-26-s1-risk-reminders/README.md`. All checked items above
...
- [x] E5 local/synthetic scope: inventory replay and event balance snapshots have zero fixture violations.
- [x] E5 local/synthetic scope: exact undo references and reverses the original per-batch consumption event.
- [x] E5 local/synthetic scope: eight identical concurrent intake retries and eight concurrent undo retries each produce one side effect; changed-payload key reuse conflicts.
