...
| 产品与药剂柜 | 搜索筛选、状态、完整资料/图片/多成分编辑、停用/删除和风险入口 | 基础列表、详情、编辑和补货已存在 | 是 |
...
同一语义必须跨页面复用同一颜色；例如“临期”不能在药剂柜使用橙色、在详情页又变成绿色。
...
| 5 | 药剂柜 | 产品列表 | 查找产品、查看详情、计划、库存、批次、成本、笔记和 AI | 今日批量执行 |
...
| L1 | 记录、计划、添加、成分、药剂柜 | 显示固定五项底栏 | 切换入口保留当前会话内的筛选、日期和滚动位置 |
...
| 药剂柜 | 搜索/筛选、产品状态、余量和风险 | 分类、批次、成本和详情入口 | 把每个产品全部字段塞进列表卡片 |
...
1. 普通用户采用“记录 / 计划 / 添加 / 成分 / 药剂柜”五项固定底栏；今日和历史合并在记录域，账户设置移出底栏。
2. 核心产品在桌面浏览器仍使用居中的约 430px 移动壳，不继承 Uni 当前桌面左侧栏；仅管理员表格页可以使用独立宽屏布局。

确认后，模块 5 将定义身份、注册、Demo 与数据控制的完整需求。
